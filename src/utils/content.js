/* Article body preparation, shared by the renderer and the table of contents
   so the two always agree on heading ids. */
...
/* Returns { html, toc } — the body with stable heading ids and wrapped tables,
   plus the headings the TOC renders. Run once on the server and pass both
   down, so the client never re-parses the article. */
